...
export default {
  ...
  actions: {
    ...
    GetWorkGroup({ state, commit }) {
      ...
      socket.on("reset", function(workGroups) {
        const indexGroup = workGroups.findIndex(function(r) {
          return r.id === Number(state.groupid);
        });

        commit("SET_SHEETS", workGroups[indexGroup].sheets);
        commit("SET_GROUP", workGroups[indexGroup]);
        commit("SET_GROUPS", workGroups);
      });
